...
const App = () => {
  ...
  // State for Gemini API response and loading status
  ...
  const handleChange = (e) => {
    ...
    // Reset API response and prediction when form changes
    setPrediction(null);
    setApiResponse(null);
  };
  ...
  // Function to call Gemini API for churn prevention strategy
  const handleGetStrategy = async () => {
      // Show loading state
      setIsLoading(true);
      setApiResponse(null);

      // Construct a prompt based on the user's input and churn prediction
      const userPrompt = `Berdasarkan data pelanggan ini (lama berlangganan: ${formData.tenure} bulan, biaya bulanan: ${formData.monthlyCharges}, layanan internet: ${formData.internetService}, kontrak: ${formData.contract}), yang berpotensi churn, berikan 3 strategi ringkas dan praktis untuk mencegah pelanggan ini berhenti berlangganan. Jawaban dalam bahasa Indonesia dan dalam bentuk poin-poin.`;
      ...
      try {
          ...
          if (result.candidates && result.candidates.length > 0 &&
            ...
            setApiResponse(text); // Set the API response to the state
          } else {
            setApiResponse("Maaf, tidak dapat menghasilkan strategi. Silakan coba lagi.");
          }
      } catch (error) {
          console.error("Error calling Gemini API:", error);
          setApiResponse("Terjadi kesalahan saat menghubungi API. Silakan periksa konsol.");
      } finally {
        ...
        <section className="bg-gray-50 p-6 rounded-lg shadow-inner">
          ...
          {prediction !== null && (
            <div className="mt-8 text-center p-4 rounded-lg bg-gray-100 border border-gray-200">
              ...
              {/* Button to get strategy using Gemini API */}
              {prediction === 'Yes' && (
                <button 
                  onClick={handleGetStrategy} 
                  disabled={isLoading}
                  className="mt-4 px-6 py-2 bg-purple-600 text-white font-bold rounded-lg shadow-md hover:bg-purple-700 focus:outline-none focus:ring-4 focus:ring-purple-500/50 transition-transform transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? 'Mengambil Strategi...' : 'Dapatkan Strategi ✨'}
                </button>
              )}
            </div>
          )}
        ...
        {apiResponse && (
          <section className="bg-purple-50 p-6 rounded-lg shadow-inner">
            ...
            <div className="prose max-w-none text-gray-700">
              <p className="whitespace-pre-line leading-relaxed">{apiResponse}</p>
            </div>
          </section>
        )}
